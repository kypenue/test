...
from cuply.series.models import SeriesModel, SESeriesModel
...
class SEStageAsyncRepo(BaseAsyncRepo):
    """ Async repository for SingleEliminationStageModel. """
    model = SingleEliminationStageModel

    def get_full_stage_query(self, *filters):
        return select(SingleEliminationStageModel).filter(*filters).options(
            selectinload(SingleEliminationStageModel.rounds).options(
                selectinload(SingleEliminationStageRoundModel.series).options(
                    selectinload(SESeriesModel.series).options(
...
class SingleEliminationStageRoundModelAsyncRepo(BaseAsyncRepo):
    """ Async repository for SingleEliminationStageRoundModel. """
    model = SingleEliminationStageRoundModel

    def get_full_round_query(self, *filters):
        return select(SingleEliminationStageRoundModel).filter(*filters).options(
            selectinload(SingleEliminationStageRoundModel.series).options(
                joinedload(SESeriesModel.series).options(
                    joinedload(SeriesModel.gamer1).options(
                        joinedload(AccountModel.user),
                        joinedload(AccountModel.game).options(
                            joinedload(GameModel.image)
                ...
                joinedload(SESeriesModel.next_winner),
            ),
        )
