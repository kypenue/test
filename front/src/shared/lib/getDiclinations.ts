...
interface GetDeclinationsProps {
    ...
    many: string;
    one: string;
    onlyDeclension?: boolean;
}

export const getDeclinations = ({ count, few, many, one, onlyDeclension = false }: GetDeclinationsProps) => {
    ...
    return onlyDeclension ? declension : `${count} ${declension}`;
};